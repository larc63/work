...
extension TMDBClient {
    ...
    */
    ...
    func getRequestToken(completionHandler: (success: Bool, requestToken: String?, errorString: String?) -> Void) {
        ...
        _ = taskForGETMethod("authentication/token/new", parameters: [:]) {(result: AnyObject!, error: NSError?) in
            guard let parsedResult = result as? NSDictionary else{
                completionHandler(success: false, requestToken: nil, errorString: "TheMovieDB returned an error")
                return
            }
            
            let status = parsedResult["success"] as! Bool
            let token = parsedResult["request_token"] as! String
            completionHandler(success: status, requestToken: token, errorString: nil)
        }
    }
